<template>
  <div class="container">
    ...
    <div v-if="loggin" class="card bg-light mt-3 mb-3">
      <div class="card-body">
        <form id="myForm" v-on:submit.prevent="enviar">
...
export default {
  ...
  created() {
    ...
    this.axios.get(uri).then(response => {
      ...
      if (token != undefined) {
        this.loggin = true;
        // Esta petición debe hacerse después
        this.axios
  ...
  methods: {
    ...
    enviar() {
      ...
      this.axios
        ...
        .then(res => {
          router.push({ name: "ficha", params: { id: this.juego._id } });
          // console.log("Devuelto");
          // window.location.reload();
          //
          // router.push(`/ficha/${this.$route.params.id}`);
          // window.location.reload();
          this.axios
            .post("/comentarios", {
              titulo: this.juego.titulo
